...
3. 时间与上下文推导（最关键规则）：
   - 必须以提供的当前时间为基准进行推算。
   - due_date 代表"任务开始时间"，而非截止时间！例如用户说"早上10点到12点修改简历"，due_date 应该是 10:00（开始时间），而不是 12:00（结束时间）。
   - 当用户提到时间范围（如"10点到12点"、"下午2点到4点"），始终取时间范围的起始时间作为 due_date。
...
4. 模糊时间段的默认分配规则（当用户只说了"早上/上午/下午/晚上"而没给具体时间时，必须遵守）：
   - "早上"或"上午"：第1个任务默认 09:30，如果有多个早上任务，依次间隔1小时（09:30, 10:30, 11:30...）。
   - "下午"：第1个任务默认 14:00，如果有多个下午任务，依次间隔1.5小时（14:00, 15:30, 17:00...）。
   - "晚上"：第1个任务默认 20:00，如果有多个晚上任务，依次间隔1小时（20:00, 21:00, 22:00...）。
   - 注意：仅在用户未指定具体数字时间时才使用默认规则。如果用户说了"下午3点"，则直接用 15:00，不要用默认值。
